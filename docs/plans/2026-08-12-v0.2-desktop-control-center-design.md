...
v0.2.0 把 OpenSimDash 从“终端 Host + 手机网页”升级为“桌面控制中心 + 手机仪表盘”。桌面端负责可发现性、安装与诊断；高频遥测仍沿用游戏 → Rust Host → 局域网 WebSocket → 手机/iPad 的数据面。运行时不需要 OpenSimDash 远程服务器，只有用户允许时的 GitHub Release 更新检查访问互联网。
...
flowchart LR
    ...
    subgraph Desktop["OpenSimDash Desktop · 单进程"]
        Lock["用户级实例锁"]
        Core["Rust Host Core"]
...
```text
OpenSimDash/
├─ OpenSimDash Desktop(.exe/.app)   # 默认有头入口，内嵌 Host
├─ opensimdash-host(.exe)           # 独立无头入口
...
`instance.lock` 不在退出时删除，避免 Unix inode 竞争；进程只持有文件句柄，操作系统在崩溃后自动释放。相邻 `instance-owner.json` 只含 PID/模式/版本/时间，供第二个进程在 Windows 锁区不可读时仍能报告持有者。成功持锁后才 bind 网络，因此错误可以区分 OpenSimDash 重复运行和第三方端口占用。`OPENSIMDASH_RUNTIME_DIR` 仅用于测试/开发隔离。
...
Steam 默认目录只用于只读建议；最终目标始终显示给用户。Windows 携带 x64 插件；macOS 的桌面应用分别提供 arm64/x86_64，但两种安装包都携带由游戏进程加载的 x86_64 SCS bridge。SDK 1.14 头文件只定义 x86/x64 插件 ABI，因此 bridge 架构不跟随 Tauri 进程架构。

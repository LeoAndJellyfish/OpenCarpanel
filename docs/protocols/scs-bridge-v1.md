...
逐字节阵列图、四款游戏完整链路和 ETS2LA 方案对照见[游戏数据链路与 SCS 44 字节协议图解](../data-paths-and-scs-packet.md)。
...
新增字段不得改变 v1 的长度或偏移；需要不兼容布局时发布新 major version，并让旧 Host 明确报告 unsupported version。

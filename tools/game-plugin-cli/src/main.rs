...
fn run() -> Result<(), Box<dyn Error>> {
    ...
    match arguments.as_slice() {
        ...
        _ => Err(
            "usage: opencarpanel-plugin pack <manifest.json> <decoder.wasm> <output.ocp-plugin>\n       opencarpanel-plugin validate <plugin.ocp-plugin>"
                .into(),
        ),
    }
}
...
fn validate(package: &Path) -> Result<(), Box<dyn Error>> {
    ...
    opencarpanel_game_plugin_runtime::WasmGameAdapter::from_bytes(
        &verified.manifest,
        &verified.module,
    )?;
    println!(
...
mod tests {
    ...
    fn packs_and_validates_the_example_manifest() -> Result<(), Box<dyn Error>> {
        ...
        let package_path = temporary.path().join("example-sim.ocp-plugin");
